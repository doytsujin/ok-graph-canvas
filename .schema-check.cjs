...
var require_dynamicAnchor = __commonJS({
  "node_modules/ajv/dist/vocabularies/dynamic/dynamicAnchor.js"(exports2) {
    ...
    function _getValidate(cxt) {
      ...
      const sch = new compile_1.SchemaEnv({ schema: schema2, schemaId, root: root2, baseId, localRefs, meta });
...
function str(v, path, out, required) {
  if (v === void 0) {
...
function validateField(input) {
  ...
  if (isObj(nodes)) {
    for (const [key, n] of Object.entries(nodes)) {
      ...
      if (n.state !== void 0 && (typeof n.state !== "string" || !STATES.has(n.state))) {
        problems.push({
          path: `${at}.state`,
  ...
  if (isObj(links)) {
    for (const [key, l] of Object.entries(links)) {
      ...
      if (l.envelope !== void 0) {
        if (!isObj(l.envelope)) {
          problems.push({ path: `${at}.envelope`, message: `expected an object, received ${describe(l.envelope)}` });
        } else {
          for (const k of ["confidence", "policy_compat", "trust", "readiness", "relearning_pressure"]) {
  ...
  if (input.projection !== void 0) str(input.projection, "projection", problems, false);
  if (input.updated_at !== void 0 && typeof input.updated_at !== "number") {
    problems.push({ path: "updated_at", message: `expected a number, received ${describe(input.updated_at)}` });
  }
  return {
    valid: problems.length === 0,
    problems,
    field: problems.length === 0 ? input : null
  };
}
...
var nested = (0, import_node_path.join)(process.cwd(), "packages/graph-canvas");
var root = (0, import_node_fs2.existsSync)((0, import_node_path.join)(nested, "package.json")) ? nested : process.cwd();
var schema = JSON.parse((0, import_node_fs.readFileSync)((0, import_node_path.join)(root, "schema/semantic-field.schema.json"), "utf8"));
var ajv = new import__.default({ strict: false, allErrors: true });
var bySchema = ajv.compile(schema);
var node = (id, extra = {}) => ({
  id,
  label: id,
  kind: "thing",
...
var bad = validateField({ nodes: { a: node("a") }, links: { l: { id: "l", source: "a", target: "ghost" } } });
